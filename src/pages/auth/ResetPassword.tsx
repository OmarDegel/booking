...
function ResetPassword() {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(errorData?.message || t("password_reset_failed"));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex flex-col lg:flex-row min-h-screen">
        <div className="hidden lg:flex w-1/2 h-screen bg-red-300 items-center justify-center p-12 relative overflow-hidden">
          <div className="text-white text-center space-y-4">
            ...
            <h2 className="text-4xl font-bold">{t("signup_title")}</h2>
        ...
        <div className="w-full lg:w-1/2 h-screen flex items-center justify-center px-6 lg:px-12">
          <div className="w-full max-w-md">
            <h2 className="text-3xl font-bold mb-2">{t("reset_password_header")}</h2>
            <p className="text-gray-400 mb-8">{t("enter_new_password")}</p>
            {step === "EMAIL" && (
              <CheckEmail
                email={email}
                setEmail={setEmail}
